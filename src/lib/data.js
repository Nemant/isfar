/* ===========================================================================
   ...
   ES-module port (Astro/Vite build): the values that used to live on
   window.ISFAR_DATA are now named exports. The runtime behaviour — the sample
   table, lookup(), lookupRemote(), useRemoteApi() — is byte-for-byte the same.
   The localStorage keys, the same-origin /api/flight call, and the
   hostname-based local-vs-remote switch are unchanged.
   =========================================================================== */
...
export function lookupRemote(raw, date) {
  const code = String(raw || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
